...
class ApiService {
    ...
    async getRealtimeQuotes(symbols: string[]): Promise<RealtimeQuotesResponse> {
        const params = new URLSearchParams({ symbols: symbols.join(',') })
        return this.request<RealtimeQuotesResponse>(`/v1/market/quotes?${params}`)
    }
